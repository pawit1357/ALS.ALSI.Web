    ...
    public partial class WD_GCMS_CVR : System.Web.UI.UserControl
    {
        ...
        private void initialPage()
        {
            ...
            if (this.tbCas != null && this.tbCas.Count > 0 && this.coverpages != null && this.coverpages.Count > 0)
            {
                ...
                gvResult.DataSource = this.tbCas.Where(x => x.row_type.Value != Convert.ToInt32(CommandNameEnum.MajorCompounds));
                gvResult.DataBind();
            }
            ...
            if (this.jobSample != null)
            {
                ...
                if (status == StatusEnum.CHEMIST_TESTING || userLogin.role_id == Convert.ToInt32(RoleEnum.CHEMIST))
                {
                    ...
                    gvCoverPages.Columns[5].Visible = true;
                    gvCoverPages.Columns[6].Visible = true;
                ...
                if (this.coverpages != null && this.coverpages.Count > 0)
                {
                    ...
                    {
                        tb_unit unit2 = new tb_unit().SelectByID(Convert.ToInt32(hProcedureUnit.Value));
                        if (unit2 != null)
                        {
                            _unit = unit2.name;
                                    ...
                                    for (int j = 15; j < isheet.LastRowNum; j++)
                                    {
                                        if (isheet.GetRow(j) != null)
                                        {
                                            ...
                                            tmp.row_type = (
                                                ...
                                                String.IsNullOrEmpty(tmp.pk) && !String.IsNullOrEmpty(tmp.library_id) && !String.IsNullOrEmpty(tmp.area)) ? Convert.ToInt32(CommandNameEnum.TotalRow) : Convert.ToInt32(CommandNameEnum.Normal);
                                ...
                                else
                                {
                                    ...
                                    for (int j = 15; j < isheet.LastRowNum; j++)
                                    {
                                        if (isheet.GetRow(j) != null && index <= 5)
                                        {
                                            ...
                                            tmp.row_type = Convert.ToInt32(CommandNameEnum.MajorCompounds);
        ...
        protected void gvCoverPages_RowEditing(object sender, GridViewEditEventArgs e)
        {
            gvCoverPages.EditIndex = e.NewEditIndex;
            gvCoverPages.DataSource = this.coverpages;
            gvCoverPages.DataBind();
            DropDownList _ddlResult = (DropDownList)gvCoverPages.Rows[e.NewEditIndex].FindControl("ddlResult");
            if (_ddlResult != null)
            {
            ...
            DropDownList _ddlPassFail = (DropDownList)gvCoverPages.Rows[e.NewEditIndex].FindControl("ddlPassFail");
            if (_ddlPassFail != null)
            {
                _ddlPassFail.Items.Clear();
                _ddlPassFail.Items.Add(new ListItem("PASS", "PASS"));
                _ddlPassFail.Items.Add(new ListItem("FAIL", "FAIL"));
            }


        }
        protected void gvCoverPages_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
        {
            gvCoverPages.EditIndex = -1;
            gvCoverPages.DataSource = this.coverpages;
            gvCoverPages.DataBind();
        }

        protected void gvCoverPages_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {

            int _id = Convert.ToInt32(gvCoverPages.DataKeys[e.RowIndex].Values[0].ToString());
            DropDownList _ddlResult = (DropDownList)gvCoverPages.Rows[e.RowIndex].FindControl("ddlResult");
            DropDownList _ddlPassFail = (DropDownList)gvCoverPages.Rows[e.RowIndex].FindControl("ddlPassFail");

            if (_ddlResult != null && _ddlPassFail != null)
            {
                template_wd_gcms_coverpage _tmp = this.coverpages.Find(x => x.ID == _id);
                if (_tmp != null)
                {
                    //=IF(C30="NA","NA",IF(D30>C30,"FAIL","PASS"))
                    _tmp.D = _ddlResult.SelectedValue;
                    _tmp.E = _ddlPassFail.SelectedValue;
                    _tmp.row_type = Convert.ToInt32(CommandNameEnum.Normal);
                }
            }
                ...
                if (gcms != null)
                {
                    switch (cmd)
                    {
                        case CommandNameEnum.Hide:
                            gcms.row_type = Convert.ToInt32(CommandNameEnum.Hide);
                            break;
                        case CommandNameEnum.Normal:
                            gcms.row_type = Convert.ToInt32(CommandNameEnum.Normal);
                            break;
                    }
                    gvCoverPages.DataSource = this.coverpages;
                    gvCoverPages.DataBind();
        ...
        protected void gvCoverPage_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                CommandNameEnum cmd = (CommandNameEnum)Enum.ToObject(typeof(CommandNameEnum), (int)gvCoverPages.DataKeys[e.Row.RowIndex].Values[1]);
        ...
        protected void gvResult_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (_cas != null)
                {
                    CommandNameEnum cmd = (CommandNameEnum)Enum.ToObject(typeof(CommandNameEnum), (int)_cas.row_type);
                    switch (cmd)
                    {
                        case CommandNameEnum.Normal:
                            e.Row.ForeColor = System.Drawing.Color.Black;
                            break;
                        case CommandNameEnum.TotalRow:
        ...
        protected void ddlSpecification_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (detailSpec != null)
            {
                ...
                foreach (DetailSpecComponent item in CustomUtils.GetComponent(headerDs, detailSpec, ignoreIndex))
                {
                    ...
                    work.A = "CVR";//Required Test
                    work.B = item.name;//Analytes
                    work.C = item.value;//Specification Limits 
                    work.D = String.Empty;
                    work.E = String.Empty;//PASS / FAIL
                    //work.RowState = this.CommandName;
                    work.row_type = Convert.ToInt32(CommandNameEnum.Normal);
        ...
        protected void lbDownload_Click(object sender, EventArgs e)
        {
            ...
            viewer.LocalReport.ReportPath = Server.MapPath("~/ReportObject/gcms_wd_cvr.rdlc");
            viewer.LocalReport.SetParameters(reportParameters);

            viewer.LocalReport.DataSources.Add(new ReportDataSource("DataSet1", dt)); // Add datasource here
            viewer.LocalReport.DataSources.Add(new ReportDataSource("DataSet2", this.coverpages.Where(x => x.row_type.Value == Convert.ToInt32(CommandNameEnum.Normal)).ToDataTable())); // Add datasource here
            viewer.LocalReport.DataSources.Add(new ReportDataSource("DataSet3", this.tbCas.FindAll(x => x.row_type == Convert.ToInt32(CommandNameEnum.MajorCompounds)).ToDataTable())); // Add datasource here
        ...
        protected void lbDownloadPdf_Click(object sender, EventArgs e)
        {
            ...
            ReportViewer viewer = new ReportViewer();
            viewer.ProcessingMode = ProcessingMode.Local;
            viewer.LocalReport.ReportPath = Server.MapPath("~/ReportObject/gcms_wd_cvr_pdf.rdlc");
            viewer.LocalReport.SetParameters(reportParameters);
