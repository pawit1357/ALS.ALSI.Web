    ...
    public partial class WD_FTIR_IDM : System.Web.UI.UserControl
    {
        ...
        protected void btnLoadFile_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < FileUpload1.PostedFiles.Count; i++)
            {
                ...
                {
                    errors.Add(String.Format("กรุณาตรวจสอบ {0}:{1} [{2}]", sheetName, CustomUtils.ErrorIndex,ex.Message));
        ...
        private void CalculateCas()
        {
            ...
            {
                if (item.B.ToLower().Contains("NVR".ToLower()))
                {
                    item.D = lbC26.Text;
                    item.E = prepareUtil(item.C, item.D);
                }
                else if (item.B.ToLower().Contains("Silicone".ToLower())){
                    item.D = txtC41.Text;
                    item.E = prepareUtil(item.C, item.D);
                }
