...
{
    public partial class Seagate_DHS : System.Web.UI.UserControl
    {
        ...
        private void initialPage()
        {
            ...
            //ddlUnit.SelectedValue = this.coverpages[0].unit+"";
            //txtDecimal01.Text = this.coverpages[0].digit1+"";
            //txtDecimal02.Text = this.coverpages[0].digit2 + "";
                ...
                case StatusEnum.CHEMIST_TESTING:
                    if (this.tbCas.Count > 0)
                    {
                        ...
                        foreach (template_seagate_dhs_coverpage _val in this.coverpages)
                        {
                            ...
                            _val.digit1 = Convert.ToInt32(txtDecimal01.Text);
                            _val.digit2 = Convert.ToInt32(txtDecimal02.Text);
                                    ...
                                    for (int j = 15; j < isheet.LastRowNum; j++)
                                    {
                                        if (isheet.GetRow(j) != null)
                                        {
                                            ...
                                            switch (compare_date)
                                            {
                                                ...
                                                case "00010011"://TOTAL
                                                    if ("Total Outgassing".Equals(tmp.classification))
                                                    {
                                                    ...
                                                    break;
                                                case "00110011"://Hydrocarbons, Unknowns, Others
                                                    tmp.row_type = Convert.ToInt32(RowTypeEnum.TotalRow);
                                                    tmp.amount = Math.Round(Convert.ToDecimal(CustomUtils.GetCellValue(isheet.GetRow(j).GetCell(7))), Convert.ToInt32(txtDecimal01.Text)).ToString();
                                                    Console.WriteLine();
                                                    break;
        ...
        private void GenerrateCoverPage()
        {
            ...
            if (this.tbCas != null && this.tbCas.Count > 0)
            {
                ...
                foreach (template_seagate_dhs_coverpage _cover in this.coverpages)
                {
                    ...
                    tmp = this.tbCas.Find(x => _cover.name.Equals(" "+x.classification.Trim()) && x.row_type == Convert.ToInt32(RowTypeEnum.TotalOutGas));
                    if (tmp != null)
                    {
                        switch (tmp.amount)
                        {
