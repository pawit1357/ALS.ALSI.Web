...
{
    public partial class WD_LPC_Unified : System.Web.UI.UserControl
    {
...
//50  LPC Particles/ part  1
        ...
        protected void btnLoadFile_Click(object sender, EventArgs e)
        {
            ...
            listOFParticle.Add("0.5");
            listOFParticle.Add("0.6");
            //listOFParticle.Add("0.700");
            //listOFParticle.Add("1.000");
            listOFParticle.Add("2");
                    ...
                    if (_postedFile.ContentLength > 0)
                    {
                        if ((Path.GetExtension(_postedFile.FileName).Equals(".xls")))
                        {
                            ...
                            using (FileStream fs = new FileStream(source_file, FileMode.Open, FileAccess.Read))
                            {
                                ...
                                if (null == isheet)
                                {
                                    isheet = wd.GetSheet("LPC");
                                }
                                txtB48.Text = CustomUtils.GetCellValue(isheet.GetRow(15 - 1).GetCell(ExcelColumn.B));
                                txtB49.Text = CustomUtils.GetCellValue(isheet.GetRow(16 - 1).GetCell(ExcelColumn.B));
                                txtB50.Text = CustomUtils.GetCellValue(isheet.GetRow(17 - 1).GetCell(ExcelColumn.B));
                                txtB51.Text = CustomUtils.GetCellValue(isheet.GetRow(18 - 1).GetCell(ExcelColumn.B));
                                ...
                                for (int row = 23; row < 48; row++)
                                {
                                    if (isheet.GetRow(row) != null) //null is when the row only contains empty cells 
                                    {
                                        ...
                                        if (listOFParticle.Contains(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B))))
                                        {
                                            switch (idx)
                                            {
                                                case 1:
                                            ...
                                            lpc.A = grp.ToString();
                                            lpc.B = (Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B))) == 2) ? "2.0" : CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B));
                                            lpc.C = CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.C));
                                            lpc.D = CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.D));
                                            //
                                            //Double _e = Math.Round(Convert.ToDouble(lpc.C), MidpointRounding.AwayFromZero);
                                            //Double _f = Math.Round(Convert.ToDouble(lpc.D), MidpointRounding.AwayFromZero);
                                            //Double result =  ((_f - _e) < 0 ? 0 : (_f - _e))*(Convert.ToDouble(txtB49.Text)/Convert.ToDouble(txtB51.Text));
                                            //
                                            lpc.E = CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.G));
                                            lpc.F = Math.Round(Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.H))), MidpointRounding.AwayFromZero).ToString();

                                            lpc.row_type = Convert.ToInt32(RowTypeEnum.Normal);
                                            lpc.data_type = Convert.ToInt32(WDLpcDataType.DATA_VALUE);
                                            tmps.Add(lpc);
                                            idx++;
                                ...
                                idx = 1;
                                String grpName = "Average";
                                for (int row = 50; row < 67; row++)
                                {
                                    ...
                                    switch (idx)
                                    {
                                        ...
                                        case 5:
                                        case 6:
                                            grpName = "Standard Deviation";
                                            break;
                                        case 7:
                                        case 8:
                                        case 9:
                                            grpName = "%RSD Deviation";
                                            break;
                                        case 10:
                                        case 11:
                                        case 12:
                                    ...
                                    if (listOFParticle.Contains(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B))))
                                    {
                                        tmp.B = (Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B))) == 2) ? "2.0" : CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.B));
                                        tmp.C = string.Empty;
                                        tmp.D = string.Empty;
                                        if(grpName.Equals("%RSD Deviation"))
                                        {
                                            tmp.E = Convert.ToDouble(Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.G))) * 100).ToString("N0");
                                            tmp.F = Convert.ToDouble(Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.H))) * 100).ToString("N0");
                                        }
                                        else
                                        {
                                            tmp.E = CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.G));
                                            tmp.F = Math.Round(Convert.ToDouble(CustomUtils.GetCellValue(isheet.GetRow(row).GetCell(ExcelColumn.H))), MidpointRounding.AwayFromZero).ToString();
                                        }
            ...
            //    tmp.C = string.Empty;
            //    tmp.D = string.Empty;
            //    tmp.E = CustomUtils.Average(lpcs.Where(x => x.data_type == Convert.ToInt32(WDLpcDataType.DATA_VALUE) && x.B.Equals(par)).Select(x => Convert.ToDouble(x.E)).ToList()).ToString();
            //    tmp.F = CustomUtils.Average(lpcs.Where(x => x.data_type == Convert.ToInt32(WDLpcDataType.DATA_VALUE) && x.B.Equals(par)).Select(x => Convert.ToDouble(x.F)).ToList()).ToString();
            //    tmp.data_type = Convert.ToInt32(WDLpcDataType.SUMMARY);
            //    lpcs.Add(tmp);
            //}
            //foreach (String par in listOFParticle)
            ...
            //    tmp.C = string.Empty;
            //    tmp.D = string.Empty;
            //    tmp.E = CustomUtils.StandardDeviation(lpcs.Where(x => x.data_type == Convert.ToInt32(WDLpcDataType.DATA_VALUE) && x.B.Equals(par)).Select(x => Convert.ToDouble(x.E)).ToList()).ToString();
            //    tmp.F = CustomUtils.StandardDeviation(lpcs.Where(x => x.data_type == Convert.ToInt32(WDLpcDataType.DATA_VALUE) && x.B.Equals(par)).Select(x => Convert.ToDouble(x.F)).ToList()).ToString();
            //    tmp.data_type = Convert.ToInt32(WDLpcDataType.SUMMARY);
            //    lpcs.Add(tmp);
            //}
            //foreach (String par in listOFParticle)
        ...
        protected void lbDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                viewer.LocalReport.ReportPath = Server.MapPath("~/ReportObject/lpc_wd_v3.rdlc");
            }
            else
            {
