...
{
    public partial class WD_GCMS_Unified : System.Web.UI.UserControl
    {
                        ...
                        if ((Path.GetExtension(_postedFile.FileName).ToLower().Equals(".xls")))
                        {
                            using (FileStream fs = new FileStream(source_file, FileMode.Open, FileAccess.Read))
                            {
                                ...
                                else
                                {
                                    ...
                                    tmp.classification = "Siloxane";
                                    tmp.amount = Math.Round(Convert.ToDecimal(CustomUtils.GetCellValue(isheet.GetRow(6).GetCell(2))), Convert.ToInt32(txtDecimal01.Text)).ToString();
                                    tmp.row_type = Convert.ToInt32(RowTypeEnum.Normal);
                                    _cas.Add(tmp);
                                    tmp = new tb_m_gcms_cas();
                                    tmp.ID = 2;
                                    tmp.sample_id = this.SampleID;
                                    tmp.classification = "Silane";
                                    tmp.amount = Math.Round(Convert.ToDecimal(CustomUtils.GetCellValue(isheet.GetRow(8).GetCell(2))), Convert.ToInt32(txtDecimal01.Text)).ToString();
                                    tmp.row_type = Convert.ToInt32(RowTypeEnum.Normal);
                                    _cas.Add(tmp);
                                    tmp = new tb_m_gcms_cas();
                                    tmp.ID = 3;
                                    tmp.sample_id = this.SampleID;
                                    tmp.classification = "Total Organic";
                                    tmp.amount = Math.Round(Convert.ToDecimal(CustomUtils.GetCellValue(isheet.GetRow(4).GetCell(2))), Convert.ToInt32(txtDecimal01.Text)).ToString();
        ...
        protected void ddlSpecification_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (detailSpec != null)
            {
                ...
                foreach (String _name in AnalytesName)
                {
                    ...
                    work.A = (index == 1 ) ? "Unified GCMS Extractable" : String.Empty;//Required Test
        ...
        private void GenerrateCoverPage()
        {
            ...
            foreach (template_wd_gcms_coverpage _cover in newCoverPage)
            {
                ...
                _cover.C = _cover.C.Equals("<0")? "0": CustomUtils.isNumber(_cover.C) ? "<" + _cover.C : _cover.C;
            }
        ...
        protected void lbDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                viewer.LocalReport.ReportPath = Server.MapPath("~/ReportObject/gcms_wd_unified.rdlc");
            }
